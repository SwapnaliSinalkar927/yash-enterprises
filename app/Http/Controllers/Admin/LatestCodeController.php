...
use Gate;
use DataTables;
use App\Models\{Code,Wholesaler,Order,WD,Brand,LatestCode};
use Illuminate\Support\Facades\DB;

class LatestCodeController extends Controller
{
   public function index(){
        if(Gate::denies('latest_code_access')) {
            return redirect()->route('admin.home')->with([
                'status' => 'failed',
                'message' => 'This action is unauthorized',
            ]);
        }
        return view('admin.latest-code.index');
    }
    public function latestCodeList(Request $request){
        $wdId = $request->get('wd_id');
        $brandId = $request->get('brand_id');

        $query = LatestCode::with(['latestWd', 'brand'])
          ->when($wdId, function ($query, $wdId) {
            return $query->where('latest_wd_id', $wdId);
        })
          ...
          ->editColumn('status', function ($code) {
            ...
            ->addColumn('wd_code', function ($code) {
                ...
                if (Gate::allows('wd_access')) {
                    $url = route('admin.wds.show', $code->latestWd->id);
                    return '<a href="' . $url . '">' . e($code->latestWd->code) . '</a>';
                }

                return e($code->latestWd->code);
            })
    ...
    public function create(){
        ...
        // $wds = WD::all();
        $brands = Brand::all();
        return view('admin.generate-code.create',compact('brands'));
    }
    ...
    public function store(GenerateCodeRequest $request){
        ...
        for ($i = 0; $i < $chunks; $i++) {
            ...
            GenerateCode::dispatch($currentChunkSize, $codesValue, $currentBatch, $brandId);
        }
