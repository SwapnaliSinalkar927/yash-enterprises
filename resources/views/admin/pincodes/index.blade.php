                ...
                <div class="hk-example card card-shadow card-wth-tabs  mt-3" >
                    <div class="card-header">
                        ...
                        <div class="page-title-right">
                                ...
                                <div class="btn-group">
                                    <button type="button" class="btn btn-primary add">Add Pincodes</button>
                                    <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split"
                                        data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="mdi mdi-chevron-down"></i>
                                    </button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('admin.pincodes.create') }}">Add
                                                pincode</a>
    ...
    <div class="modal fade" id="uploadCSV" tabindex="-1" role="dialog" aria-labelledby="uploadCSVTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.bulk_pincodes_upload') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-12">
                                <p>Download a <a href="{{ asset('sample_csv/pincodes.csv') }}" download>sample CSV template</a> to see an example of the format required.</p>
                                <div class="mt-3">
                                    <label for="formFile" class="form-label">Import CSV</label>
                                    <input type="hidden" name="table" value="pincodes">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $("#example").DataTable({
                ...
                ajax: "{{ route('admin.pincodes.list') }}",
                columns: [
                    { 
                        data: 'code', 
                        name: 'code',
                        render: function(data, type, row) {
                            var code = row.code;
                            @can('pincode_show')
                                var showUrl = '{{ route('admin.pincodes.show',':pincodeID') }}'.replace(':pincodeID', row.id);
                    ...
                    { 
                        ...
                        render: function(data, type, row) {
                            var state_name = row.city.state.name;
                            @can('state_show')
                                var showUrl = '{{ route('admin.states.show',':stateID') }}'.replace(':stateID', row.city.state.id);
                                state_name = '&emsp;<a href="'+showUrl+'" class="mr-3 text-info" data-toggle="tooltip" data-placement="top" title="View" data-original-title="View">'+row.city.state.name+'</a>';
                            @endcan
                            return state_name;
                        }
                    },   
                    ...
                    { 
                        ...
                        render: function(data, type, row) {
                            ...
                            var show = '';
                            @can('pincode_edit')
                                var editURL = '{{ route('admin.pincodes.edit',':pincodeID') }}'.replace(':pincodeID', row.id);
