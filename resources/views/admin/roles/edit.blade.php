                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') ?? $role->name }}" id="name" name="name" required>
                                                    @error('name')
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug') ?? $role->slug }}" id="slug" name="slug" required readonly>
                                                    @error('slug')
                                                    <div class="invalid-feedback">
                                                        {{ $message }}
                                                    </div>
                                                    ...
                                                    <ul class="permissions" id="permissions">
                                                        @foreach($permissions as $permission)
                                                            @if(!$permission->parent_id)
                                                                <li>
                                                                    <input type="checkbox" id="{{ $permission->id }}" name="permissions[]" value="{{ $permission->id }}" @if(in_array($permission->id, old('permissions', $role->permissions->pluck('id')->toArray()))) checked @endif>
                                                                    {{ $permission->name }}
                                                                    @if($permission->children->count() > 0)
                                                                        @include('admin.roles.partials.edit_permission_children', ['children' => $permission->children, 'permission_ids'   => $role->permissions->pluck('id')->toArray()])
                                                                    @endif
                                                                </li>
                                                            @endif
                                                        @endforeach
                                ...
                                <div class="card card-wth-line">
                                    ...
                                    <div class="card-body mb-1">
                                        ...
                                        <select name="status" id="activeStatus" class="form-select">
                                            <option value="1" {{ $role->status == "1" || old('status') == "1" ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ $role->status == "0" || old('status') == "0" ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                    </div>
                                    <div class="card-footer text-muted"></div>
                                </div>
