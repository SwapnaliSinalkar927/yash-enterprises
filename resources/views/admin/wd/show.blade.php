...
<div class="hk-pg-wrapper">
    <div class="container-fluid p-5 pb-0">
        ...
        WD's Detail
            {!! ($wd->status==1) ? '<span class="badge badge-sm badge-success">Active</span>' : '<span class="badge badge-sm badge-danger">Inactive</span>' !!}
            @can('wd_edit')
            <a href="{{ route('admin.wds.edit', $wd->id) }}" class="mr-3 text-primary" data-toggle="tooltip"
                    ...
                    <div class="col-md-5">
                         <div class="row">
                            <div class="col-md-6 mb-3">
                                <div class="card-line bg-primary table-card-line"></div>
                                <a href="{{ route('admin.generate-codes.index',['wd_id' =>$wd->id ]) }}">
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="card-line bg-primary table-card-line"></div>
                                <a href="{{ route('admin.orders.index',['wd_id' =>$wd->id ]) }}">
                                    <div class="card card-shadow mb-0">
                                        <div class="card-header card-header-action">
                                            <h6>Total Orders</h6>
                                        </div>
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="card-line bg-primary table-card-line"></div>
                                <a href="{{ route('admin.orders.index',['wd_id' =>$wd->id, 'brand_id'=> $brand->brand_id]) }}">
                                    <div class="card card-shadow mb-0">
                                        <div class="card-header card-header-action">
    ...
    <script>
        $( document ).ready(function() {
            ...
            var table = $("#example").DataTable({
                ...
                ajax: "{{ route('admin.wd.orders.list', ['wd_id' => $wd->id])  }}",
                columns: [
                    { 
            ...
            var table = $("#code").DataTable({
                ...
                ajax: "{{ route('admin.wd.codes.list', ['wd_id' => $wd->id])  }}",
                columns: [
                     {
                        data: 'code',
                        name: 'code',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if(row.brand_id){
                                var brandCode = row.brand.name;
                                @can('brand_show')
                                var url = '{{ route('admin.brands.show',':brandID') }}'.replace(':brandID', row.brand_id);
                                brandCode = '<a href="'+url+'">'+row.brand.name+'</a>'
                                @endcan
                                return brandCode;
                            }
                            else{
